...
function outpoint($row,$index){
	...
	if (!empty($row[6])){
		...
		printf("</LineString>\n</Placemark>");
		// coll
		$key=sprintf("%f_%f_%d",$row[6],$row[7],$row[8]);
		if (!isset($icon[$key])){
			printf("<Placemark><name>col: %s</name><styleUrl>#col</styleUrl><Style><IconStyle><heading>%f</heading></IconStyle></Style>\n",$row[8],headings($row[1],$row[2],$row[6],$row[7]));
