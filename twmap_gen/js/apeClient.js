var APE = {
	...
	Client: function(server,core) {
			params = server.match(/^([^\.]+)\.(.*)$/);
			this.frequency = params[1];
...
APE.Client.prototype.load = function(config){
	...
	config.frequency = config.frequency || this.frequency;
	...
	config.server = config.server || this.server;
	...
	//if (tmp) {
		...
		//config.frequency = (config.frequency + 1) % 2;
	//} else {
		cookie = '{"frequency":0}';
	//}
...
APE.Config.baseUrl = mydir; //APE JSF 
//APE.Config.domain = '0.ape.happyman.dev.xuite.net'; 
APE.Config.domain = 'auto'; 
...
(function(){
	for (var i = 0; i < arguments.length; i++)
		APE.Config.scripts.push(APE.Config.baseUrl + 'ape_js/' + arguments[i] + '.js');
