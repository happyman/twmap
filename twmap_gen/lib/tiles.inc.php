	...
	for($j=$a[1];$j<=$b[1];$j++) {
		for ($i=$a[0]; $i<=$b[0]; $i++) {
			...
			if (!file_exists("$dir/$imgname")) {
				// create tile cache from Internet
				// exec(sprintf("wget -q -O %s 'http://rs.happyman.idv.tw/map/tw25k2001/zxy/${zoom}_${i}_${j}.png'","$dir/$imgname"));
				$download[] = sprintf("wget -c --tries=0 --read-timeout=2 -O %s $tileurl","$dir/$imgname",$zoom,$i,$j);
...
function img_from_tiles3($x, $y, $shiftx, $shifty, $zoom, $ph=0, $debug=0, $options) { // $tmpdir="/dev/shm", $tileurl, $image_ps_args=array() ) {
