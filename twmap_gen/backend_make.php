...
$cmd = sprintf("php cmd_make2.php -r %d:%d:%d:%d -O %s -v %d -t '%s' -i %s -p %d %s -l %s:%s %s %s %s", $startx, $starty, $shiftx, $shifty, $outpath, $version, addslashes($title), $_SERVER['REMOTE_ADDR'], $ph, $svg_params, $_SESSION['mylogin']['email'], $inp['formid'], isset($inp['grid_100M']) ? '-e' : '',
 // 是否包含 100M grid
