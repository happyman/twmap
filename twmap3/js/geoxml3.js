    ...
    doc.styles = styles;
      ...
      for (pm = 0; pm < placemarkNodes.length; pm++) {
        ...
        node = placemarkNodes[pm];
				// happyman 
				if (node.parentNode.nodeName == 'Folder'
					&& geoXML3.nodeValue(node.getElementsByTagName('name')[0]) == 'Path') {
					 placemarkname = node.parentNode.getElementsByTagName('name')[0];
				 } else {
					 placemarkname = node.getElementsByTagName('name')[0];
				 }
...
var createPolyline = function(placemark, doc) {
  ...
  if (!parserOptions.suppressInfoWindows) {
    var infoWindowOptions = geoXML3.combineOptions(parserOptions.infoWindowOptions, {
      content: '<div class="geoxml3_infowindow"><h3>' + placemark.name + 
               '</h3><div>' + placemark.description + parserOptions.additional_path_desc +'</div></div>',
      pixelOffset: new google.maps.Size(0, 2)
    });
    if (parserOptions.infoWindow) {
...
var createPolygon = function(placemark, doc) {
  ...
  if (!parserOptions.suppressInfoWindows) {
    var infoWindowOptions = geoXML3.combineOptions(parserOptions.infoWindowOptions, {
      content: '<div class="geoxml3_infowindow"><h3>' + placemark.name + 
               '</h3><div>' + placemark.description + parserOptions.additional_polygon_desc +'</div></div>',
      pixelOffset: new google.maps.Size(0, 2)
    });
    if (parserOptions.infoWindow) {
      p.infoWindow = parserOptions.infoWindow;
