...
var TaiwanMapV1Options = {
	getTileUrl: function (a, b) {
		return "//gis.sinica.edu.tw/tileserver/file-exists.php?img=TM25K_1989-jpg-" + b + "-" + a.x + "-" + a.y;
	},
...
var TaiwanMapOptions = {
	getTileUrl: function (coord, zoom) {
		return "//tile.happyman.idv.tw/map/tw25k2001/" + zoom + "/" + coord.x + "/" + coord.y + ".png";
	},
	tileSize: new google.maps.Size(256, 256),
	maxZoom: 17,
	minZoom: 10,
...
var GPXTrackOptions = {
	getTileUrl: function (a, b) {
		return '//tile.happyman.idv.tw/map/gpxtrack/' + b + "/" + a.x + "/" + a.y + ".png";
	},
	tileSize: new google.maps.Size(256, 256),
	maxZoom: 19,
	minZoom: 10,
...
var MOI_OSM_Options = {
	getTileUrl: function (a, b) {
		return "//tile.happyman.idv.tw/map/rudy/" + b + "/" + a.x + "/" + a.y + ".png";
	},
	tileSize: new google.maps.Size(256, 256),
	maxZoom: 20,
	name: "魯地圖",
...
var RUDY_BN_Options = {
	getTileUrl: function (a, b) {
		return "//tile.happyman.idv.tw/map/rudy_bn/" + b + "/" + a.x + "/" + a.y + ".png";
	},
	tileSize: new google.maps.Size(256, 256),
	maxZoom: 21,
	name: "魯地圖",
...
var RUDY_DN_Options = {
	getTileUrl: function (a, b) {
		return "//tile.happyman.idv.tw/map/rudy_dn/" + b + "/" + a.x + "/" + a.y + ".png";
	},
	tileSize: new google.maps.Size(256, 256),
	maxZoom: 21,
	name: "魯地圖",
...
var RUDY_TN_Options = {
	getTileUrl: function (a, b) {
		return "//tile.happyman.idv.tw/map/rudy_tn/" + b + "/" + a.x + "/" + a.y + ".png";
	},
	tileSize: new google.maps.Size(256, 256),
	maxZoom: 21,
	name: "魯地圖",
...
var Happyman_Options = {
        getTileUrl: function (a, b) {
                return "//tile.happyman.idv.tw/map/happyman/" + b + "/" + a.x + "/" + a.y + ".png";
        },
        tileSize: new google.maps.Size(256, 256),
        maxZoom: 21,
        name: "Happyman航跡圖",
...
var Compartment_Options = {
        getTileUrl: function (a, b) {
                return "//tile.happyman.idv.tw/map/forest/" + b + "/" + a.x + "/" + a.y + ".png";
        },
        tileSize: new google.maps.Size(256, 256),
        maxZoom: 21,
        name: "林班界圖",
...
var Hillshading_Options = {
	getTileUrl: function (a, b) {
		return '//tile.happyman.idv.tw/map/colorrelief/' + b + "/" + a.x + "/	" + a.y + ".png";
	},
	tileSize: new google.maps.Size(256, 256),
	maxZoom: 21,
	name: "陰影",
...
var Fandi_Options = {
	getTileUrl: function (a, b) {
		return "//gis.sinica.edu.tw/tileserver/file-exists.php?img=JM50K_1916-jpg-"+b+"-"+a.x+"-"+a.y;
	},
	tileSize: new google.maps.Size(256, 256),
	name: "蕃地",
...
var JM50K1924_Options = {
	getTileUrl: function (a, b) {
		return "//gis.sinica.edu.tw/tileserver/file-exists.php?img=JM50K_1924-jpg-"+b+"-"+a.x+"-"+a.y;
	},
	tileSize: new google.maps.Size(256, 256),
	name: "陸測",
...
var TM50K1966_MapType = new google.maps.ImageMapType({
		getTileUrl: function (a, b) {
			return "//gis.sinica.edu.tw/tileserver/file-exists.php?img=TM50K_1966-jpg-"+b+"-"+a.x+"-"+a.y;
		},
		tileSize: new google.maps.Size(256, 256),
		name: "水利圖",
...
var ATISMapType = new google.maps.ImageMapType({
		getTileUrl: function (tile, zoom) {
			...
			*/
			return "//tile.happyman.idv.tw/map/atis/" + zoom + "/" + tile.x + "/" + tile.y + ".png";
		},
		tileSize: new google.maps.Size(256, 256),
		maxZoom: 21,
		minZoom: 10,
...
var JM20K1921_MapType = new google.maps.ImageMapType({
		getTileUrl: function (tile, zoom) {
			...
			if (tileBounds.intersects(LY_Bounds)) {
				...
				return "//tile.happyman.idv.tw/map/lanyu1921/" + zoom + "/" + tile.x + "/" + y_tms + ".png";
			}
...
var JM20K1904_MapType = new google.maps.ImageMapType({
		getTileUrl: function (tile, zoom) {
			...
			if (tileBounds.intersects(LY_Bounds)) {
				var y_tms = (1 << zoom) - tile.y - 1;
				return "//tile.happyman.idv.tw/map/lanyu1904/" + zoom + "/" + tile.x + "/" + y_tms + ".png";
			}
...
var copyrights = {
	...
	'fandi': "<a target=\"_blank\" href=\"http://gissrv4.sinica.edu.tw/gis/twhgis.aspx\">台灣歷史百年地圖</a> - 五萬分之一蕃地地形圖 1907~1916",
	'jm50k': "<a target=\"_blank\" href=\"http://gissrv4.sinica.edu.tw/gis/twhgis.aspx\">台灣歷史百年地圖</a> - 日治五萬分之一(陸地測量部 1924~1944)",
	...
	'atis': "<a target=\"_blank\" href=\"http://gissrv4.sinica.edu.tw/gis/twhgis.aspx\">台灣歷史百年地圖</a> - 農航所正射影像",
...
function showInsideMarkers() {
	...
	var j = 0;
	var i;
	if (show_label === 0 || tags_ready === 0 || markers_ready === 0)
		return;
...
function showGrid(grid_type) {
	...
	if (!bigPoly)
		bigPoly = new google.maps.Rectangle({
				map: map,
				fillColor: "red",
				fillOpacity: 0.1
			});
...
function showCWBRainfall(fcast_type) {
	if (rainkml && rainkml.term) {
		if (rainkml.term == fcast_type || rainkml.loading == 1)
			return;
...
function locInfo_show(newpos, ele, extra) {
	...
	if (ele > -1000)
		content += "<br>高度: " + ele.toFixed(0) + "M";
	content += "<br>座標: " + comment + "<br>" + comment2 + "<br>" + comment3;
	// /* allow from all points
	// if (ele > -1000)
	content += "<br><a href=# id='los_link' onClick='javascript:show_line_of_sight(" + newpos.toUrlValue(5) + "," + ele.toFixed(0) + ")'><img id=\"los_eye_img\"  title='通視模擬' src=img/eye.png width=32/></a>";
...
function smarker_connect(p2) {
	...
	if (surveyMarker && surveyMarker.getMap()) {
		...
		/*jshint evil:true */
		var jsonObject = (myshapes) ? eval("(" + myshapes + ")") : {
			"shapes": []
		};
...
function showCenterMarker(name) {
	...
	if (name === '') {
		// alert("請輸入");
		showmeerkat(pointdata_url + "?lastest=10&err=1", {
			'width': '600'
		});
		return;
	}
	...
	if (posxy) {
		...
		// hook POI search
		showmeerkat(poisearch_url + "?name=" + encodeURIComponent(name), {
			width: 600
		});
		...
		success: function (data) {
			if (data.ok === true) {
				...
				if (data.rsp.is_tw === 0) {
					...
					showmeerkat(pointdata_url + "?lastest=10&err=4", {
						'width': '600'
					});
					return false;
				}
				...
				geocoder.geocode({
					...
					if (status == google.maps.GeocoderStatus.OK) {
						...
						if (p === 0) {
							//alert("不在台澎範圍");
							showmeerkat(pointdata_url + "?lastest=10&err=2", {
								'width': '600'
							});
							return false;
						}
						...
						showmeerkat(pointdata_url + "?lastest=10&err=3", {
							'width': '600'
						});
						return false;
					}
...
function cleanDelaunayTriangulation(class_num) {
	if (typeof delaunayGMpolys[class_num] === 'undefined')
		return;
...
function mysetIcon2(type, isShadow) {
	...
	if (theme == 'ingress') {
		...
		if (type == "一等點")
			return 'img/ingress/enl_' + Math.floor((Math.random() * 8) + 1) + 'res.png';
		else if (type == "二等點")
			return 'img/ingress/enl_' + Math.floor((Math.random() * 8) + 1) + 'res.png';
		else if (type == "三等點")
			return 'img/ingress/enl_' + Math.floor((Math.random() * 8) + 1) + 'res.png';
...
function initialize() {
	...
	var GreenstyledMap = new google.maps.StyledMapType(green_styles, {
			name: "theme"
		});
	...
	if (!is_mobile) {
		...
		shapesMap = new ShapesMap($("#delete-button")[0], $("#clear-button")[0], $("#shapeinfo-button")[0],
				...
				showmeerkat(get_elev_url + "?infoshapes=1", {
					'width': '600'
				});
			},
				function (shapes) {
	...
	// map-specific events
	document.getElementById("map_canvas").addEventListener('dragenter', showPanel,false);
	...
	var map_is_ready = google.maps.event.addListener(map, "bounds_changed", function () {
			...
			var st = {
				"show_label": getParameterByName("show_label"),
...
function markerReloadSingle(opt) {
	...
	if (opt.action == 'update') {
		...
		allmarkers[to_update_id].setOptions({
...
path: [{ lng: parseFloat(badlines[i][0].lng),lat: parseFloat(badlines[i][0].lat)},{  lng: parseFloat(badlines[i][1].lng),lat: parseFloat(badlines[i][1].lat) } ],
strokeOpacity: 0,
icons: [{
icon: lineSymbol,
offset: '0',
